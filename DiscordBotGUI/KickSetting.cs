...
{
    public partial class KickSetting : Form
    {
        ...
        public KickSetting(ILogger logger, DiscordSocketClient client)
        {
            InitializeComponent();
            _logger = logger ?? throw new ArgumentNullException(nameof(logger), "[KickSetting] Loggerインスタンスはnullにできません!!");
            _client = client;
            _logger?.Log("-------------------- [KickSetting] フォームが初期化されました!! --------------------", (int)LogType.Debug);
        }
        //コントロールの有効/無効状態を更新
        private void UpdateControlsState()
        {
            ...
            int delayMs = RegistryHelper.LoadKickDeleteDelayMs();
        ...
        //フォームロード時の処理
        private void KickSetting_Load(object sender, EventArgs e)
        {
            _logger?.Log("[INFO] KickSetting_Loadイベントを開始!!", (int)LogType.Debug);
            ...
            {
                bool shouldDelete = RegistryHelper.LoadKickShouldDelete();
            ...
            {
                int timeoutMinutesKick = RegistryHelper.LoadKickTimeoutMinutes();
                NumTimeoutMinutes.Value = timeoutMinutesKick;
                _logger?.Log($"[INFO] NumTimeoutMinutes (TimeoutMinutes) を [{timeoutMinutesKick} 分]に設定!!", (int)LogType.Debug);
            }
            //コントロールの有効/無効
            _logger?.Log("[INFO] UpdateControlsStateメソッドを呼び出し!!", (int)LogType.Debug);
            UpdateControlsState();
            _logger?.Log("[INFO] KickSetting_Loadイベントを終了!!", (int)LogType.Debug);
        }
        ...
        private void LoadCurrentSettings(ulong guildId)
        {
            ...
            //サーバーIDに対応する設定を読み込む
            List<ulong> allowedRoleIds = RegistryHelper.LoadKickAllowedRoleIds(guildId);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            int delayMs = RegistryHelper.LoadKickDeleteDelayMs();
            int timeoutMinutesKick = RegistryHelper.LoadKickTimeoutMinutes();
            _logger?.Log($"[INFO] 既存の設定値：ShouldDelete=[{shouldDelete}], DelayMs=[{delayMs}], TimeoutMin=[{timeoutMinutesKick}]", (int)LogType.Debug);
            if (!(CmbGuilds.SelectedItem is GuildDisplayItem selectedGuild))
            {
                ...
                bool settingChanged =
                    ...
                    timeoutMinutesKick != (int)NumTimeoutMinutes.Value;
                if (settingChanged)
                {
                    _logger?.Log("[INFO] ギルドは選択されていませんが、グローバル設定に変更がありました!! 保存処理を実行します!!", (int)LogType.Debug);
                    //レジストリにプロンプトMSG自動削除フラグ保存
                    RegistryHelper.SaveKickShouldDelete(ChkAutoDeleteEnabled.Checked);
                    //レジストリにプロンプトMSG自動削除時間を保存
                    RegistryHelper.SaveKickDeleteDelayMs((int)NumDeleteDelayMs.Value);
                    //レジストリに対話中のプロンプトMSGタイムアウト時間を保存
                    RegistryHelper.SaveKickTimeoutMinutes((int)NumTimeoutMinutes.Value);
            ...
            else
            {
                ...
                // レジストリにサーバーIDと紐づけて保存
                RegistryHelper.SaveKickAllowedRoleIds(selectedGuild.GuildId, roleIdsToSave);
        ...
        private void SaveFormSettings()
        {
            ...
            if (Properties.Settings.Default.FormSetting == true)
            {
                ...
                Properties.Settings.Default.KickSetting_FormX = width;
                Properties.Settings.Default.KickSetting_FormY = height;
                Properties.Settings.Default.KickSetting_PositionX = formPosition.X;
                Properties.Settings.Default.KickSetting_PositionY = formPosition.Y;
        ...
        //フォームアクティベート時
        private void KickSetting_Activated(object sender, EventArgs e)
        {
            if (!isActivated)
            {
                //メソッド開始ログ
                _logger.Log($"[INFO] KickSetting_Activatedイベントを開始!!", (int)LogType.Debug);
                _logger.Log($"[INFO] フォームアクティベート処理中...", (int)LogType.Debug);
                _logger.Log($"[INFO] フォーム初回起動：[{!isActivated}]", (int)LogType.Debug);
                _logger.Log($"[INFO] フォームの終了位置記憶設定：[{Properties.Settings.Default.FormSetting}]", (int)LogType.Debug);
                _logger.Log($"[INFO] フォームの初期化フラグ：[{Properties.Settings.Default.KickSetting_Init}]", (int)LogType.Debug);
                int formX = Properties.Settings.Default.KickSetting_FormX;
                int formY = Properties.Settings.Default.KickSetting_FormY;
                int positionX = Properties.Settings.Default.KickSetting_PositionX;
                int positionY = Properties.Settings.Default.KickSetting_PositionY;
                if (Properties.Settings.Default.FormSetting == true)
                {
                    if (Properties.Settings.Default.KickSetting_Init == true)
                    {
                        //フォームの初期サイズを設定
                        ...
                        Properties.Settings.Default.KickSetting_Init = false;
                        _logger.Log($"[INFO] フォームの初期化フラグを[{Properties.Settings.Default.KickSetting_Init}]に設定!!", (int)LogType.Debug);
                        Properties.Settings.Default.Save();
                        isActivated = true;
                    }
                    else
                ...
                //メソッド終了ログ
                _logger.Log($"[INFO] KickSetting_Activatedイベントを終了!!", (int)LogType.Debug);
            }
        }
        //フォームクローズ時
        private void KickSetting_FormClosing(object sender, FormClosingEventArgs e)
        {
            //メソッド開始ログ
            _logger.Log($"[INFO] KickSetting_FormClosingイベントを開始!!", (int)LogType.Debug);
            _logger.Log($"[INFO] SaveFormSettingsメソッドを呼び出し!!", (int)LogType.Debug);
            SaveFormSettings();
            //メソッド終了ログ
            _logger.Log($"[INFO] KickSetting_FormClosingイベントを終了!!", (int)LogType.Debug);
            _logger.Log("-------------------- [KickSetting] フォーム終了処理完了!! --------------------", (int)LogType.Debug);
        }
    }
}
